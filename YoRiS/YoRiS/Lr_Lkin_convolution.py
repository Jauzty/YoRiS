...
def KLF_FRI_FRII(Lrr, z, LR):
    ...
    if __name__ == "__main__":
        ...
        #plt.plot(Rx_values, PP)
        #plt.plot(Lr_values, np.log10(Phir21), label= 'FRI')
        plt.plot(kin, np.log10(Phikinscatter2), linestyle = '--', label = 'FRI')
        ...
        plt.plot(kkin, np.log10(PhikinFRIIscatter2), linestyle = ':', color = 'C0', label = 'FRII')
        ...
        plt.plot(kin_interp, np.log10(Phikintotscatter2), linestyle = '-', color = 'black', label = 'Total Kinetic Output')
        plt.legend(frameon=False, ncol=1, fontsize=12.5)
        ax.grid()
        ax.set_xlabel('$ \log L_{k} [erg s^{-1}] $', fontsize = 15)
        ax.set_ylabel(r'$\log \Phi(L_{k}) \; [Mpc^{-3} \; dex^{-1}]$', fontsize = 15)
        #plt.title('Total Kinetic Luminosity Function ($z = 0.5$)')
