...
for i, (zz, file_name) in enumerate(zip(zvalues, file_list)): #take the qlf files and separate them based on their z
    ...
    """def objective_function(gk, kkin, LgLbol_target):
        Lbolfrii = -np.log10(gk) + kkin #both kin and Lglbol are logged so we have to have loggk in the eq
        residuals = Lbolfrii - LgLbol_target
        chi_squared_value = np.sum(residuals**2)
        return chi_squared_value
    gk_init = 0.1
    LgLbol_target = np.interp(PhikinFRII21, PhiBbol, Lboloptdata)-4.3
    # Run optimization for gk alone
    result = minimize(objective_function, gk_init, args=(kkin, LgLbol_target), method='Nelder-Mead')
    print(result.x)
